...
def kmeans_plot(datafile):
    ...
    kmeans_cluster(df_array, m, labels)
    return()
...
def kmeans_cluster(df_array, m, labels):
   ...
   # spectral.fit(X) # build the model 
    ...
    plot_kmeans = plot_dir+'/kmeans_image.png'
    #bytes_image = io.BytesIO()
    
    plt.savefig(plot_kmeans, format='png')
    
    with Image.open(plot_kmeans) as image:
        image.show()
...
def user_kmeans(df_array, m, labels):
 ...
 #   plt.scatter( X[:,0],X[:,1], alpha=0.25, cmap = 4 )
  ...
  #  plt.show()
    ...
    user_kmeans = plot_dir+'/user_image.png'
    plt.savefig(user_kmeans, format = 'png')
    
    with Image.open(user_kmeans) as image:
        image.show()
